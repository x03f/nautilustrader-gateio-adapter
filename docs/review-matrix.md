...
exception that unfroze the fresh-cache restart leaves an instrument with an adopted-order booking
entirely unguarded, and the erasure survives there (`REC-07`, open and blocking). The sweep's
placement, which the withdrawn repair got wrong and this one got right, was never in question:
order state is correct on both routes across the whole refutation matrix.
...
**`REC-06` edges, found by the eighth round's audit (below the blocking bar, open as limitations):**
the order report's average price (`avg_deal_price`/`fill_price`) still rides a forgiving reader —
an unreadable stated average is reported as no average, and the engine prices any inferred stand-in
fill it builds from that report at the order's limit price or at zero; a spot fill that states a
fee without a readable `fee_currency` books the commission in the quote currency, which misstates
the commission's denomination when the venue charged it in base; and the spot status arithmetic
still infers `finished` for a stream payload stating neither `status` nor `event` — no documented
venue payload reaches that branch, but it contradicts the round's own rule that absence makes no
claim. Each is a strict-read candidate for the next pass at this surface.
