...
| `STOP_MARKET` | `POST /spot/price_orders` | `POST /futures/{settle}/price_orders` | `POST /delivery/usdt/price_orders` | denied | Implemented and mock-tested |
| `STOP_LIMIT` | as above | as above | as above | denied | Implemented and mock-tested |
| `MARKET_IF_TOUCHED` | as above | as above | as above | denied | Implemented and mock-tested |
| `LIMIT_IF_TOUCHED` | as above | as above | as above | denied | Implemented and mock-tested |
...
*Gate.io* did. Everything this adapter refuses on its own is a denial, whether
...
order = self.order_factory.trailing_stop_market(
    ...
    emulation_trigger=TriggerType.DEFAULT,   # DEFAULT == BID_ASK
)
...
|---|---|---|
| `LIMIT`, `STOP_MARKET`, `MARKET_IF_TOUCHED`, `TRAILING_STOP_MARKET` | yes | `MARKET` |
| `STOP_LIMIT`, `LIMIT_IF_TOUCHED`, `TRAILING_STOP_LIMIT` | yes | `LIMIT` |
| `MARKET`, `MARKET_TO_LIMIT` | no | — |
...
This section describes the platform, not this package: the behaviour above was
...
| `display_qty=0` (fully hidden) | denied | denied | denied | denied | Implemented and mock-tested |
...
column says whether the suite also pins the behaviour.
...
| `post_only` combined with `IOC` or `FOK` on a limit order | `OrderDenied` — `poc` rests until cancelled, so the immediacy cannot survive the substitution | Implemented and mock-tested |
